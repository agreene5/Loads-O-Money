...
@export var exp_stretch_scale: float = 5.5
...
func _ready() -> void:
		exp_title_label = Label.new()  # Create the new label
		exp_label = Label.new()
		next_label = Label.new()
		job_label = Label.new()
		
		add_child(exp_title_label)  # Add the new label first
		add_child(exp_label)
...
func update_font_size() -> void:
		...
		if font is FontFile:
				...
				for label in [exp_title_label, next_label, job_label]:  # Added exp_title_label
				...
				bold_font_variation.set_variation_embolden(1.0)
				...
				exp_label.scale.x = exp_stretch_scale
		else:
